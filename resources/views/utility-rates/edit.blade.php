    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
            <div class="p-6">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-700 dark:text-gray-400">
                            Update rates for water and electricity utilities
                        </p>
                    </div>
                ...
                <form action="{{ route('utility-rates.update', $utilityRate) }}" method="POST" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="p-6 space-y-6">
                        <div>
                            ...
                            <div class="relative mt-1">
                                <input type="date" name="effective_date" id="effective_date" required
                                    value="{{ old('effective_date', $utilityRate->effective_date->format('Y-m-d')) }}"
                            ...
                            <div>
                                ...
                                <div class="relative mt-1 rounded-lg shadow-sm">
                                    <input type="number" name="water_rate" id="water_rate" required
                                        min="0" step="0.01" value="{{ old('water_rate', $utilityRate->water_rate) }}"
                            ...
                            <div>
                                ...
                                <div class="relative mt-1 rounded-lg shadow-sm">
                                    <input type="number" name="electric_rate" id="electric_rate" required
                                        min="0" step="0.01" value="{{ old('electric_rate', $utilityRate->electric_rate) }}"
                                        class="block w-full pl-10 pr-12 border-gray-300 dark:border-gray-700 dark:bg-gray-900 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg py-2.5 text-gray-800 dark:text-gray-100">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <svg class="h-5 w-5 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm">{{ old('notes', $utilityRate->notes) }}</textarea>
                        </div>
                    </div>
...
<script>
    flatpickr('input[type="date"]', {
        dateFormat: 'Y-m-d',
        defaultDate: '{{ $utilityRate->effective_date->format("Y-m-d") }}',
